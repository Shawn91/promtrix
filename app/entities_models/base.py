...
class LLMResponse(MySQLModel, table=False):
    ...
    id: UUID = Field(default_factory=uuid4, primary_key=True)
    content: str
    role: ResponseRole = Field(description="user or assistant role. future role may include agent, etc")
...
class LLMInteraction(MySQLModel, table=False):
    ...
    category: LLMInteractionCategory = Field(
        ...
        description="What is this interaction for? Evaluating llm response or having llm/prompt generate a response?",
    )
...
class EvaluationStep(MySQLModel, table=False):
    """
    Represents a step in the evaluation process of a response/choice of an llm request.
    """

    execution_id: UUID | None = Field(
        default=None,
        description="The id of the execution that generated this step. For example, the id of the llm interaction that is used for this evaluation step.",
    )
    method: EvaluationMethod = Field(description="The name of the step")
...
class Evaluation(MySQLModel, table=False):
    ...
    metric: EvaluationMetric = Field(description="The metric used for the evaluation")
    created_at: datetime = Field(
